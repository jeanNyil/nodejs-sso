...
1. Retrieve the user ${ACCESS_TOKEN}
    ...
    https://sso.apps.cluster-00ea.sandbox567.opentlc.com/auth/realms/nodejs-example/protocol/openid-connect/token \
...
6. Create an non-secure route to expose the `nodejs-sso` RESTful service outside the OpenShift cluster.
    ...
    --hostname=nodejs-sso.apps.cluster-00ea.sandbox567.opentlc.com
    ```
...
```
curl -v -w '\n' \
http://nodejs-sso.apps.cluster-00ea.sandbox567.opentlc.com/securePing \
...
<
* Connection #0 to host nodejs-sso.apps.cluster-00ea.sandbox567.opentlc.com left intact
{"message":"You did succeed to call the secure route ! :)"}
* Closing connection 0
```