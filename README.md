...
1. Retrieve the user ${ACCESS_TOKEN}
    ...
    https://sso.apps.jeannyil.sandbox1047.opentlc.com/auth/realms/nodejs-example/protocol/openid-connect/token \
...
6. Create an non-secure route to expose the `nodejs-sso` RESTful service outside the OpenShift cluster.
    ...
    --hostname=nodejs-sso.apps.jeannyil.sandbox1047.opentlc.com
    ```
...
```zsh
curl -v -w '\n' \
http://nodejs-sso.apps.jeannyil.sandbox1047.opentlc.com/securePing \
-H "Authorization: Bearer ${ACCESS_TOKEN}"
```
```zsh
*   Trying 3.122.27.192...
* TCP_NODELAY set
* Connected to nodejs-sso.apps.jeannyil.sandbox1047.opentlc.com (3.122.27.192) port 80 (#0)
> GET /securePing HTTP/1.1
> Host: nodejs-sso.apps.jeannyil.sandbox1047.opentlc.com
> User-Agent: curl/7.64.1
> Accept: */*
> Authorization: Bearer ey[...]
>
...
<
* Connection #0 to host nodejs-sso.apps.jeannyil.sandbox1047.opentlc.com left intact
{"message":"You did succeed to call the secure route ! :)"}
* Closing connection 0
```
...
```zsh
http -v GET http://nodejs-sso.apps.jeannyil.sandbox1047.opentlc.com/securePing \
